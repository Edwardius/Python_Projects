...
while(cont == True):
    ...
    numberObservations = int(input())
    dataArray = [[0 for i in range(2)] for i in range(numberObservations)]

    for x in range(numberObservations):
        dataPoint = input().split()
        for y in range(2):
            dataArray[x][y] = int(dataPoint[y])
    ...
    for x in range(numberObservations):
        checker = x + 1
        while checker < numberObservations:
            displace = 0
            time = 0
            if dataArray[x][0] > dataArray[checker][0]:
                displace = dataArray[x][1] - dataArray[checker][1]
                time = dataArray[x][0] - dataArray[checker][0]
            else:
                displace = dataArray[checker][1] - dataArray[x][1]
                time = dataArray[checker][0] - dataArray[x][0]

            speed = abs(displace/time)
            if minSpeed < speed:
                minSpeed = speed
            checker += 1
    print(minSpeed)
